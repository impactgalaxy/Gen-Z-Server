...
async function run() {
  try {
    ...
    const messCosts = client.db("Gen-Z").collection("MessCosts");
    ...
    app.post("/messCosts", async (req, res)=>{
      const doc = req.body;
      const result = await messCosts.insertOne(doc);
      res.send(result);
    })

    app.get("/mess-costs", async(req, res)=>{
      const result = await messCosts.find().toArray();
      res.send(result);
    });
